 ...
 */
async function fetchJobDetails(role, onProgress) {
  const roleId = role[0];
  ...
  try {
    ...
    if (onProgress) onProgress(roleId, roleTitle, result);
    return result;
  } catch (err) {
...
async function crawlAllCareers(roles, options = {}) {
  ...
  const results = await processWithConcurrency(
    toProcess,
    CONCURRENT_REQUESTS,
    (role, i) => fetchJobDetails(role, (id, title, data) => {
      if (onProgress) onProgress(i + 1, toProcess.length, data);
    })
  );
